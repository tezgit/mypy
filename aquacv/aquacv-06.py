...
def console(pilimage, over):
    ...
    pw = pilimage.width
    myiplen = len(myip)
    draw.rectangle(((pw - fontsize, ph - hh + lineh * 1), (pw - myiplen*fontsize,
                                                           ph - hh + lineh * 1 + fontsize)), fill=(255, 255, 255, 100))
    over.text((pw-myiplen*fontsize + 2 * fontsize, ph - hh + lineh*1),
              myip, (200, 200, 200), font=font)
...
while(cap.isOpened()):                    # play the video by reading frame by frame
    ...
    if ret == True:
        ...
        frame = rescaleFrame(videocap, 1.0)
        ...
        constext[2] = str(edges[11, 21])  # get pixel value

        # ====== check video position and loop if last frame ========
        if curframe >= (frame_count - 1):
            cap.set(cv2.CAP_PROP_POS_FRAMES, 0)

        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2BGRA)  # COLOR_BGR2RGB
        ...
        x = cv2.Sobel(gray, ddept, 1, 0, ksize=5, scale=1)
        y = cv2.Sobel(gray, ddept, 0, 1, ksize=5, scale=1)
        absx = cv2.convertScaleAbs(x)
        absy = cv2.convertScaleAbs(y)
        edge = cv2.addWeighted(absx, 0.5, absy, 0.5, 0)

        # coverlay = edge.copy()
        # cv2.rectangle(coverlay, (200, 10), (400, 80), (0, 0, 0, 200), -1)
        # alpha = 1
        # edge = cv2.addWeighted(coverlay, alpha, edge, 1 - alpha, 0)
        # edge = cv2.addWeighted(coverlay, 0.5, gray, 0.5, 0)
        ...
        if cons:
            ...
            odraw.rectangle(
                ((0, pil_im.height - hh), (pil_im.width, pil_im.height)), fill=(0, 0, 0, TezOSC.TRANSPA))
            # draw console texts
            console(pil_im, odraw)
            # composite all into pil image
            pil_im = Image.composite(pil_im, overlay, overlay)
            # edge = cv2.addWeighted(coverlay, 0.5, gray, 0.5, 0)

        # ====== SHOW CV composite image ========
        # reconvert pil image into cv2 image
        cv2_im_processed = cv2.cvtColor(np.array(pil_im), cv2.COLOR_RGB2BGR)
        # main show image
        ...
        if k == 27:    # Esc key to stop
            ...
            # curframe = int(cap.get(cv2.CAP_PROP_POS_FRAMES))
            # < (frame_count - int(frame_count / 100)):
            if curframe > (int(frame_count / 100)):
