...
class tpix:
    ...
    def tmove(self):
        self.tx += self.incx * \
            (self.dirx / random.randint(1, 10)) + math.sin(self.tx / 10.)
        self.ty += self.incy * \
            (self.diry / random.randint(1, 10)) + math.cos(self.ty / 10.)
...
while True:
    ...
    if (num < Tmaxelements):
        ...
        Tarray[num] = tpix(rx, ry, 200, 200, 200)
        num += 1
    # else:
    #     num = 0
    #     surf.fill(black)
