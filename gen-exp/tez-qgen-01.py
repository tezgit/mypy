...
class quboz:
    def __init__(self, num, x, y, w, h):
        ...
        self.incx = random.randint(0, 100) / 200.
        self.incy = random.randint(0, 100) / 200.

        # pyg.draw.circle(surf, self.mycolor, (int(self.tx), int(self.ty)), 1, 0)
        # gfxdraw.

    def qmove(self):
        self.sininc += 0.15
        self.tx += self.incx * self.dirx + \
            1 * math.sin(self.alf / random.randint(1, 10))
        self.ty += self.incy * self.diry + \
            1 * math.cos(self.alf / random.randint(1, 10))

        rrr = 150 - ((self.sininc * 1.2) % 150)

        ggg = (self.sininc * 1.3) % 150

        bbb = 0  # (self.sininc * 1) % 200

        self.mycolor = (rrr, ggg, bbb, 0)

        radiuz = int(0.1 + (5 / self.sininc))
        pyg.draw.circle(surf, self.mycolor,
                        (int(self.tx), int(self.ty)), radiuz*2, int(radiuz / 5))
...
pyg.display.set_caption('TEZQUBGEN')
screen.fill(black)
...
for nn in range(Tmaxelements):
    ...
    Tarray.append(quboz(nn, int(width / 2), int(height / 2),
