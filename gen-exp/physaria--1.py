...
def makeDir(newdir):
    # subst = cfind("/" + newdir, '/')
    # print('subst ' + str(subst))
    # curdir = str(newdir[0: subst[len(subst) - 1]])
    # print('curdir ' + curdir)
    # ftp.cwd(curdir)
...
def genstart():
   ...
   # save to remote ftp server
    # remotedir = "/physaria/" + pdate
    remotedir = pdate
    if (gnum <= 1):
        ftp.cwd("/physaria")
        makeDir(remotedir)
        # ftp.cwd(remotedir)
    remotefilename = fname + ".png"
    if(gnum > 1):
        placeFile(filename, remotefilename)

    # textsurface = myfont.render(datetime + filename.split('/', 1)[1], False, (0, 0, 0))

    rr = random.randint(20, 90)
    SAVETIME = 2 * rr * 1000
    pyg.time.set_timer(TEZTIME_EVENT, SAVETIME)
...
ftpConnect('phonomena.net', 'phonotez.23')
makeDir('physaria')
