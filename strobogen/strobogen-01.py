...
def strobe():
    ...
    x1 = width / strobemargin
    x2 = width - x1*2
    y1 = height / strobemargin
    y2 = height - y1*2

    if (strobeflag):
        pyg.draw.rect(surf, (0, 0, 0, 200), (x1, y1, x2, y2))
        pass
    else:
        pyg.draw.rect(surf, (100, 100, 100, 200), (x1, y1, x2, y2))
        pass
...
def check_events():
    ...
    for event in pyg.event.get():
        ...
        if (event.type == TEZTIME_EVENT):
            # genstart()
            print("teztime")

        if (event.type == STROBETIME_EVENT):
            strobe()
...
while True:
    ...
    # screen.blit(oversurf, (0, 0))

    # pyg.draw.rect(surf, (20, 20, 20, 50),
